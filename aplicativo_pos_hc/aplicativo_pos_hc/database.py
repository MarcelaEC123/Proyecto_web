...
def actualizar_producto(id_producto, codigo, descripcion, categoria, nombre_proveedor, valor_unitario, unidad_medida, stock):
    try:
        db_connection, cursor = conectar_bd()
        sql = "UPDATE producto SET  descripcion = %s, categoria = %s, nombre_proveedor = %s, valor_unitario = %s, unidad_medida = %s, stock = %s WHERE id_producto = %s"
        data = (descripcion, categoria, nombre_proveedor, valor_unitario, unidad_medida, stock, id_producto, codigo)
        cursor.execute(sql, data)
        db_connection.commit()
        cursor.close()
        db_connection.close()
